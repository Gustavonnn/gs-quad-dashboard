...
interface KanbanCardModalProps {
  ...
  onSaveBriefing: (cardId: string, briefingData: any) => void
  onSaveManual: (cardId: string, manualData: any) => void
}
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <Badge variant={card.type === 'hybrid' ? 'secondary' : 'warning'}>
              {card.type === 'hybrid' ? (
                <span className="flex items-center gap-1"><Brain size={10} /> HYBRID</span>
              ) : (
                <span className="flex items-center gap-1"><Zap size={10} /> MANUAL</span>
              )}
            </Badge>
          </div>
          <button
        ...
        {card.categoria && (
          <div className="flex items-center gap-1 mb-3">
            ...
            <span className="font-mono text-[10px] text-[var(--color-gs-muted)]">{card.categoria}</span>
          </div>
        )}
              ...
              <>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-[var(--color-gs-bg)] border border-[var(--color-gs-border)] rounded-md p-4 space-y-3">
                    {wf?.sugestao_titulo && (
                      <div>
                        <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Título Sugerido</div>
                        <div className="font-mono text-xs text-[var(--color-gs-text)]">{wf.sugestao_titulo}</div>
                      </div>
                    )}

                    {wf?.descricao_template && (
                      <div>
                        <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Descrição</div>
                        <div className="font-mono text-xs text-[var(--color-gs-text)] leading-relaxed">{wf.descricao_template}</div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Preço Atual</div>
                        <div className="font-mono text-sm font-bold text-[var(--color-gs-text)]">
                          {wf?.preco_atual ? `R$ ${wf.preco_atual.toFixed(2)}` : '—'}
                        </div>
                      </div>
                      <div>
                        <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Preço Recomendado</div>
                    ...
                    {wf?.elasticidade !== null && wf?.elasticidade !== undefined && (
                      <div>
                        <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Elasticidade</div>
                        <div className="font-mono text-xs font-bold text-[var(--color-gs-yellow)]">
                          ε = {wf.elasticidade.toFixed(2)}
                          <span className="text-[var(--color-gs-muted)] ml-2 font-normal">({wf.elasticidade_class ?? '—'})</span>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div className="bg-[var(--color-gs-bg)] border border-[var(--color-gs-border)] rounded-md p-3 text-center">
                        <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest">Meta</div>
                        <div className="font-mono text-sm font-bold text-[var(--color-gs-green)] mt-1">
                          R$ {wf.resultado_esperado.meta_receita.toLocaleString('pt-BR')}
                        </div>
                      </div>
                      <div className="bg-[var(--color-gs-bg)] border border-[var(--color-gs-border)] rounded-md p-3 text-center">
                        <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest">Unidades</div>
                        <div className="font-mono text-sm font-bold text-[var(--color-gs-text)] mt-1">
                          {wf.resultado_esperado.meta_unidades}
                        </div>
                      </div>
                      <div className="bg-[var(--color-gs-bg)] border border-[var(--color-gs-border)] rounded-md p-3 text-center">
                        <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest">Prazo</div>
                        <div className="font-mono text-sm font-bold text-[var(--color-gs-text)] mt-1">
            ...
            {!card.manual_data ? (
              ...
              <div className="space-y-3">
                {card.manual_data.titulo && (
                  <div>
                    <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Título</div>
                    <div className="font-mono text-xs text-[var(--color-gs-text)]">{card.manual_data.titulo}</div>
                  </div>
                )}
                {card.manual_data.descricao && (
                  <div>
                    <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Descrição</div>
                    <div className="font-mono text-xs text-[var(--color-gs-text)] leading-relaxed">{card.manual_data.descricao}</div>
                  </div>
                )}
                {card.manual_data.preco && (
                  <div>
                    <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Preço</div>
                    <div className="font-mono text-sm font-bold text-[var(--color-gs-green)]">R$ {card.manual_data.preco.toFixed(2)}</div>
                  </div>
                )}
                {card.manual_data.tags?.length > 0 && (
                  <div>
                    <div className="font-mono text-[9px] text-[var(--color-gs-muted)] uppercase tracking-widest mb-1">Tags</div>
                    <div className="flex flex-wrap gap-1">
