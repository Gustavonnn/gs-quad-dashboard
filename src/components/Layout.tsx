    ...
    >
      {({ isActive }) => (
        <>
          ...
          {isActive && (
            <div
              className="absolute left-0 top-1/2 -translate-y-1/2 w-[2px] h-4 bg-[var(--color-gs-green)] rounded-r"
          ...
          {item.accent === 'green' && !isActive && (
            <div
              className="absolute top-1.5 right-1.5 w-1 h-1 rounded-full bg-[var(--color-gs-green)] opacity-70"
          ...
          <div className="shrink-0 w-10 h-full flex items-center justify-center">
            <span
              className={`transition-all duration-200 ${isActive ? 'text-white' : 'text-[var(--color-gs-muted)]'}`}
            >
              {item.icon}
            </span>
          </div>
          ...
          >
            ...
            {item.sublabel && (
              <span
                className={`text-[9px] font-mono tracking-wide mt-0.5 whitespace-nowrap truncate ${isActive ? 'opacity-60' : 'text-[var(--color-gs-muted)] opacity-70'}`}
              >
                {item.sublabel}
              </span>
            )}
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            className="font-mono text-[8px] tracking-[0.22em] uppercase mt-1 whitespace-nowrap"
      ...
      >
        ...
        <div
          style={{
            height: '1px',
            background: 'var(--color-gs-border)',
            margin: '2px 0',
            opacity: 0.5,
          }}
        />
      ...
      >
        ...
        >
          <div
            className="font-mono text-[9px] font-bold tracking-[0.18em] uppercase whitespace-nowrap"
            style={{ color: 'var(--color-gs-green)' }}
          >
            SYSTEM ONLINE
          </div>
          <div
            className="font-mono text-[8px] tracking-widest mt-0.5 whitespace-nowrap"
            style={{ color: 'var(--color-gs-muted)' }}
          >
            LATENCY 42ms · WS
          </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className="w-8 h-8 flex items-center justify-center font-heading font-black text-[11px] uppercase"
              style={{ background: 'var(--color-gs-blue)', color: 'white', borderRadius: '2px' }}
            >
              GS
            </div>
            <div>
              <div
                ...
                style={{ color: 'var(--color-gs-text)', lineHeight: 1 }}
              >
                GS-QUAD
              </div>
              <div
        ...
        >
          {[
            ...
            <div key={label}>
              <div
                className="font-mono text-[8px] tracking-[0.25em] uppercase px-2 mb-2 opacity-40"
                style={{ color: 'var(--color-gs-muted)' }}
              >
                {label}
              </div>
                        ...
                        {isActive && (
                          <div
                            ...
                            style={{
                              ...
                              boxShadow: '0 0 6px var(--color-gs-green)',
                            }}
                          />
                        )}
                        <span className={isActive ? 'text-white' : 'text-[var(--color-gs-muted)]'}>
                          {item.icon}
                        </span>
                        <div className="flex flex-col">
                          <span>{item.label}</span>
                          {item.sublabel && (
                            <span className="text-[8px] capitalize font-normal opacity-60 mt-0.5">
                              {item.sublabel}
                            </span>
                          )}
                        </div>
              ...
              <div
                className="my-3"
                style={{ height: '1px', background: 'var(--color-gs-border)', opacity: 0.4 }}
              />
            </div>
          ))}
        </nav>
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          onMouseEnter={(e) => {
            (e.currentTarget as HTMLButtonElement).style.borderColor = 'var(--color-gs-muted)';
            (e.currentTarget as HTMLButtonElement).style.color = 'var(--color-gs-text)';
          }}
          onMouseLeave={(e) => {
            (e.currentTarget as HTMLButtonElement).style.borderColor = 'var(--color-gs-border)';
            (e.currentTarget as HTMLButtonElement).style.color = 'var(--color-gs-muted)';
          }}
        >
          <Command size={12} />
          <span>Buscar</span>
...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--color-gs-deep)', color: 'var(--color-gs-text)' }}
    >
      <DesktopSidebar />
