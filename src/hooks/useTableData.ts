...
export function useCurvaABC(limit = 500) {
  return useQuery<CurvaABC[]>({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((row) => curvaABCSchema.parse(row))
    },
    staleTime: 60 * 1000,
  })
}
...
export function useGrowthPlans(limit = 8) {
  ...
  return useQuery<IAGrowthPlan[]>({
    ...
    queryFn: async () => {
      ...
      if (error) throw error
      return (data || []).map((row) => iaGrowthPlanSchema.parse(row))
    },
    staleTime: 30 * 1000,
  })
}
...
export function useMLInsights(limit = 200) {
  return useQuery<MLInsight[]>({
    ...
    queryFn: async () => {
      ...
      if (error) throw error
      return (data || []).map((row) => mlInsightSchema.parse(row))
    },
    staleTime: 60 * 1000,
  })
}
...
export function usePriceTimeline(limit = 100) {
  return useQuery<MLPriceTimeline[]>({
    ...
    queryFn: async () => {
      ...
      if (error) throw error
      return (data || []).map((row) => mlPriceTimelineSchema.parse(row))
    },
    staleTime: 60 * 1000,
  })
}
...
export function usePaginatedAlertas({ page, pageSize, severity, status, search }: PaginatedAlertasOptions) {
