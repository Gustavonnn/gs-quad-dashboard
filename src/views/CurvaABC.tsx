...
export function CurvaABC() {
  ...
  const grupos = {
    A: safeData.filter((r) => r.curva_abc === 'A'),
    B: safeData.filter((r) => r.curva_abc === 'B'),
    C: safeData.filter((r) => r.curva_abc === 'C'),
  };
  const receitaA = grupos.A.reduce((s, r) => s + (r.receita_30d ?? 0), 0);
  const receitaB = grupos.B.reduce((s, r) => s + (r.receita_30d ?? 0), 0);
  const receitaC = grupos.C.reduce((s, r) => s + (r.receita_30d ?? 0), 0);
    ...
    <div className="flex flex-col gap-8 animate-fade-in">
      <div className="flex flex-col gap-1 mb-2">
        ...
        <p className="font-mono text-xs text-gs-muted tracking-widest uppercase">
          DISTRIBUIÇÃO 80/15/5 — PORTFÓLIO
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-5 flex flex-col gap-2">
          <span className="font-mono text-[10px] text-gs-muted tracking-widest uppercase">
            CURVA A
          </span>
      ...
      <div className="bg-gs-panel border border-gs-border rounded-sm shadow-xl flex flex-col overflow-hidden w-full">
        <div className="px-5 py-3 border-b border-gs-border flex items-center justify-between bg-gs-bg/50">
          <div className="flex items-center gap-3">
            <span className="text-[10px] text-gs-muted font-mono font-bold tracking-widest uppercase">
              PORTFOLIO_MATRIX_DB
            </span>
          </div>
          <span className="text-[10px] text-gs-muted font-mono font-bold">
            {filtered.length} SKUS FOUND
          </span>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gs-bg border-b border-gs-border">
                <th className="px-5 py-3 font-mono text-[10px] uppercase tracking-widest text-gs-muted">
                  RANK
                </th>
                <th className="px-5 py-3 font-mono text-[10px] uppercase tracking-widest text-gs-muted">
                  SKU
                ...
                <th className="px-5 py-3 font-mono text-[10px] uppercase tracking-widest text-gs-muted text-right">
                  RECEITA 30D
                </th>
                <th className="px-5 py-3 font-mono text-[10px] uppercase tracking-widest text-gs-muted">
                  CICLO
            ...
            <tbody className="bg-gs-panel divide-y divide-gs-border">
              ...
              {!loading && filtered.length === 0 && (
                <tr>
                  <td
                    colSpan={7}
                    className="px-4 py-12 text-center font-mono text-xs text-gs-muted tracking-widest uppercase"
                  >
              ...
              {filtered.map((item) => (
                <tr key={item.id} className="hover:bg-gs-bg/50 transition-colors group">
                  <td
                    className={`px-5 py-3 font-bold font-display text-sm ${CURVA_COLOR[item.curva_abc ?? 'C'] ?? 'text-gs-muted'}`}
                  >
                    {item.curva_abc ?? 'C'}
                  </td>
                  <td className="px-5 py-3 font-mono text-[11px] text-gs-text group-hover:text-white transition-colors">
                    {item.id}
                  </td>
                  <td className="px-5 py-3 text-sm text-gs-text/80 max-w-[280px] truncate">
                    {item.titulo ?? '—'}
                  </td>
                  <td className="px-5 py-3 text-right font-mono text-[11px] text-gs-green font-bold">
                    {formatCurrency(item.receita_30d ?? 0)}
                  </td>
                  <td className="px-5 py-3 font-mono text-[11px] text-gs-muted">
                    {item.ciclo ?? '—'}
                  </td>
                  <td
                    className={`px-5 py-3 font-mono text-[11px] uppercase tracking-wide ${
                  ...
                  >
                    {TENDENCIA_ICON[item.tendencia ?? 'estavel'] ?? '—'}{' '}
                    <span className="ml-1 opacity-70">{item.tendencia ?? '—'}</span>
                  </td>
