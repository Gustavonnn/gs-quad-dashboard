...
export function DataExplorer() {
  ...
  const tabs: { id: TabType; label: string; count: number; loading: boolean }[] = [
    ...
    { id: 'ia_alertas', label: 'IA Alertas', count: alertasData?.length ?? 0, loading: alertasLoading },
    { id: 'ml_insights', label: 'ML Insights', count: insightsData?.length ?? 0, loading: insightsLoading },
  ]
    ...
    () => [
      {
        ...
        size: 140,
        cell: ({ row }) => (
          <span className="font-mono text-xs text-[var(--color-gs-green)]">{row.original.id}</span>
        ),
      },
      {
        accessorKey: 'titulo',
        ...
        size: 300,
        cell: ({ row }) => (
          <span className="text-sm truncate max-w-[280px] block" title={row.original.titulo}>
            {row.original.titulo}
          </span>
        ),
      },
      {
        ...
        size: 80,
        cell: ({ row }) => {
          const c = row.original.curva_abc
          return (
      ...
      },
      {
        ...
        size: 110,
        cell: ({ row }) => {
          const t = row.original.tendencia
          const colors: Record<string, string> = {
            crescendo: 'text-[var(--color-gs-green)]',
            caindo: 'text-[var(--color-gs-red)]',
    ...
    () => [
      {
        ...
        size: 140,
        cell: ({ row }) => (
          <span className="font-mono text-xs text-[var(--color-gs-green)]">{row.original.sku}</span>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Badge
              variant={
                s === 'CRÍTICO' ? 'danger' : s === 'ALTO' ? 'warning' : s === 'MÉDIO' ? 'default' : 'secondary'
              }
            >
              {s}
            </Badge>
      ...
      },
      {
        ...
        header: 'Tipo',
        size: 150,
        cell: ({ row }) => <span className="text-xs">{row.original.tipo_alerta}</span>,
      },
      {
  ...
  const insightColumns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      },
      {
        ...
        size: 100,
        cell: ({ row }) => {
          const v = row.original.rupture_risk
          return (
            <span
              className={`font-mono text-xs font-bold ${
                v > 0.6
                  ? 'text-[var(--color-gs-red)]'
                  : v > 0.3
      ...
      },
      {
        ...
        size: 90,
        cell: ({ row }) => (
          <span className="font-mono text-xs text-[var(--color-gs-red)]">
            {row.original.anomaly_score?.toFixed(3)}
          </span>
        ),
      },
      {
        ...
        header: 'Cluster',
        size: 90,
        cell: ({ row }) => <Badge variant="secondary">{row.original.ml_cluster}</Badge>,
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-mono text-xs">
            ε = {row.original.elasticity?.toFixed(2) ?? '—'}
          </span>
        ),
      },
    ],
