    ...
    <div className="flex flex-col gap-8 animate-fade-in">
      <div className="flex flex-col gap-1 mb-2">
        ...
        <p className="font-mono text-xs text-gs-muted tracking-widest uppercase">
          DECISÕES TÁTICAS GERADAS POR IA — SUPABASE SYNC
        </p>
      </div>
      ...
      {!isLoading && (!data || data.length === 0) && (
        <div className="bg-gs-panel border border-gs-border rounded-sm shadow-xl flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-gs-text/80 font-mono text-sm tracking-wider">
            Sem planos táticos exportados na nuvem.
          </p>
          <p className="text-gs-muted text-[10px] uppercase tracking-widest">
            Aguardando execução de Geraldo (Python)
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {(data ?? []).map((plan) => (
          <Card
            key={plan.id}
            className="p-6 flex flex-col group hover:border-gs-muted/30 transition-colors duration-300"
          >
            <div className="flex items-start justify-between mb-4 border-b border-gs-border pb-3">
              <span className="text-sm font-mono font-bold text-gs-green uppercase tracking-wide">
                # {plan.sku}
              </span>
              <Badge
                variant={
                  plan.status_intervencao === 'APROVADO'
                    ? 'success'
                    : plan.status_intervencao === 'CONCLUÍDO'
                      ? 'secondary'
                      : 'warning'
                }
              >
            ...
            <p className="text-xs text-gs-yellow font-mono font-bold mb-3 uppercase tracking-wider">
              {'>>>'} {plan.acionavel}
            </p>
            <p className="text-sm text-gs-text/80 leading-relaxed">{plan.descricao_plano}</p>
            <button
              onClick={() => handleStatusChange(plan.id, plan.status_intervencao ?? 'PENDENTE')}
