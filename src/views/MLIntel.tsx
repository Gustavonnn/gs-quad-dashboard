...
export function MLIntel() {
    ...
    return (
      <div className="flex h-[80vh] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="font-mono text-sm tracking-widest text-gs-green animate-pulse">PROCESSING NEURAL DATA...</span>
        </div>
      </div>
    )
  }
  ...
  // Separar dados
  const ruptures = data.filter(d => (d.rupture_risk || 0) > 0.6).sort((a, b) => b.rupture_risk - a.rupture_risk)
  const anomalies = data.filter(d => (d.anomaly_score || 0) < 0).sort((a, b) => a.anomaly_score - b.anomaly_score)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="col-span-1 md:col-span-4 bg-gs-panel border border-gs-border p-5 rounded-sm shadow-[0_0_15px_rgba(28,233,4,0.05)] border-l-4 border-l-gs-green">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="font-mono text-xs text-gs-muted mt-2 tracking-wide max-w-3xl">
                Sistema avançado de Machine Learning. Modelos ativos: Holt-Winters (Séries Temporais), Isolation Forest (Detecção de Anomalias), K-Means (Clustering ABC) e Linear Regression (Elasticidade).
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="pl-3">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <span className="font-mono text-[10px] text-gs-muted block truncate max-w-[150px]" title={item.titulo}>{item.titulo}</span>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="font-mono text-xs text-gs-yellow font-bold">{formatPct(item.rupture_risk)}</span>
                  ...
                  <div className="mt-3 flex justify-between items-center pt-2 border-t border-gs-border/50">
                    <div className="font-mono text-[10px] text-gs-muted">
                      FORECAST 7D: <span className="text-white font-bold">{Math.round(item.forecast_7d || 0)} UN</span>
                    </div>
        ...
        <div className="bg-gs-panel border border-gs-border rounded-sm flex flex-col h-[65vh]">
          <div className="p-4 border-b border-gs-border bg-gs-bg flex justify-between items-center">
            <h3 className="font-display font-bold text-sm tracking-wide text-gs-red flex items-center gap-2">
              <AlertTriangle size={16} />
              ANOMALIAS DETECTADAS
            </h3>
            <span className="font-mono text-[10px] bg-gs-red/10 text-gs-red px-2 py-1 rounded-sm">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <span className="font-mono text-xs text-gs-text font-bold block flex items-center gap-2">
                      {item.sku}
                      <span className="text-[9px] bg-gs-red text-white px-1 rounded-sm uppercase">{item.anomaly_severity}</span>
                    </span>
                    <span className="font-mono text-[10px] text-gs-muted mt-1 uppercase text-gs-red block">
                      {item.anomaly_type?.replace('_', ' ')}
                    </span>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="font-mono text-xs text-gs-red font-bold">{item.anomaly_score?.toFixed(3)}</span>
        ...
        <div className="bg-gs-panel border border-gs-border rounded-sm flex flex-col h-[65vh]">
          ...
          <div className="p-4 overflow-y-auto flex-1 space-y-4">
            ...
            <div className="font-mono text-[10px] text-gs-muted uppercase tracking-widest mb-1">Divergências ABC (ML vs Pareto)</div>
            {clusters.length === 0 ? (
              ...
              <div key={item.sku} className="border border-gs-border bg-gs-bg p-3 rounded-sm flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[10px] text-gs-muted line-through">C: {item.curva_abc}</span>
                  <span className={`font-mono text-[10px] px-1.5 py-0.5 rounded-sm font-bold ${item.abc_divergence === 'PROMOVIDO' ? 'bg-gs-green/20 text-gs-green' : 'bg-gs-red/20 text-gs-red'}`}>
                    {item.abc_divergence}
                  </span>
                </div>
              </div>
            ...
            {data.filter(d => d.elasticity !== null && d.elasticity < -1).sort((a,b) => (a.elasticity || 0) - (b.elasticity || 0)).slice(0, 5).map((item: MLInsight) => (
              <div key={item.sku} className="border border-gs-border bg-gs-bg p-3 rounded-sm">
                ...
                <div className="flex justify-between items-center">
                  <span className="font-mono text-[10px] text-gs-muted uppercase">{item.price_sensitivity?.replace('_', ' ')}</span>
                  <span className="font-mono text-xs font-bold text-gs-yellow">ε = {item.elasticity?.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
