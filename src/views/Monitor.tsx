...
export function Monitor({ onSelectSku }: MonitorProps) {
  ...
  const hasNoAlerts = !isAnyLoading && (alertas?.length ?? 0) === 0 && (stockData?.length ?? 0) === 0
            ...
            {(stockData ?? []).map((item) => (
              ...
              >
                ...
                <div className="mt-2 text-xs font-mono text-gs-red animate-pulse flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-gs-red rounded-full" />
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <span className={`font-mono text-sm font-bold tracking-wider uppercase ${SEV_TEXT[alerta.severity] ?? 'text-gs-muted'}`}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {!isResolved && (
                        <button
                          ...
                          className="group relative px-3 py-1 bg-transparent border border-gs-green text-gs-green rounded-sm overflow-hidden transition-all hover:shadow-[0_0_15px_rgba(0,255,102,0.2)] disabled:opacity-40 disabled:cursor-not-allowed"
                        >
